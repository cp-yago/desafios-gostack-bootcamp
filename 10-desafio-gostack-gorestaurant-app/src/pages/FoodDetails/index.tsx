...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      const { id, name, description, price, image_url } = response.data;
      ...
      // Handle with extras
      const unformattedExtras = response.data.extras as Extra[];

      const formattedExtras = unformattedExtras.map(extra => {
        const tempObj = {} as Extra;
  ...
  const cartTotal = useMemo(() => {
    ...
    if (totalExtras > 0 || foodQuantity > 0) {
      totalPrice = totalExtras + foodQuantity * food.price;
    }
